...
const DentistForm = ({ dentist }) => {
    const [name, handleNameChange] = useInput(dentist.name);
    const [address, handleAddressChange] = useInput(dentist.address[0]);

    const handleSubmit = async (e) => {
        e.preventDefault()
        try {
            const data = await performAPIRequest('api/admin/dentists', 'PUT', {
